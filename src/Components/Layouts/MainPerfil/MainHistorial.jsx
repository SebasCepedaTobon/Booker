...
export const MainHistorial = () => {
  ...
  const peticionGet = () => {
    for (let index = 0; index < reservas.length; index++) {
      axios
        .get(urlEjem + reservas[index].id_libro)
        .then((response) => {
  ...
  const peticionPost = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        Swal.fire("Reserva realizada exitosamente", "", "success");
      ...
      <div className="datos-perfil">
        ...
        <div className="info-reservas">
          {reservas.length === 0 ? (
            ...
            <>
              {reservas.map((libro) => (
                <Checkoud key={libro.id} libro={libro} />
              ))}
              <button className="btn-confirmar-reserva" onClick={reservar}>
                Confirmar Reserva
              </button>
            </>
          )}
